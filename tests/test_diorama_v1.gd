...
const DioramaClass = preload("res://scripts/ui/farm_diorama_v4.gd")
...
func _init() -> void:
    ...
    _ok(int(diorama.get("visual_pass")) == 2,"runtime renderer declares visual pass 2")
    _ok(str(diorama.get("visual_target_id")) == "satoyama-premium-2026-09-15","renderer is locked to approved visual target")
    ...
    if camera is Camera3D:
        ...
        _ok(camera.size <= 11.1,"V4 camera keeps the miniature world visually dominant")
    ...
    _ok(world_root != null and world_root.name == "SatoyamaDioramaV4","V4 world root is active")
    _ok(world_root != null and world_root.get_node_or_null("IslandBase") != null,"V4 keeps the floating miniature base")
    _ok(world_root != null and world_root.get_node_or_null("StreamBridge") != null,"V4 keeps the stream bridge landmark")
    _ok(world_root != null and world_root.get_node_or_null("BackTerrace") != null,"V4 keeps layered satoyama terrain")
    var env_node = world_root.get_node_or_null("PremiumEnvironment") if world_root != null else null
    _ok(env_node is WorldEnvironment,"V4 installs premium environment lighting")
    var key_light = world_root.get_node_or_null("WarmKeyLight") if world_root != null else null
    _ok(key_light is DirectionalLight3D,"V4 installs named warm key light")
    if key_light is DirectionalLight3D:
        _ok(key_light.light_energy < 1.0,"V4 key light prevents iPhone WebGL washout")
    ...
    if farm_map != null:
        _ok(str(farm_map.get_script().resource_path).ends_with("farm_diorama_v4.gd"),"runtime uses visual-pass-2 diorama v4")

    print("3D DIORAMA V4 VISUAL CONTRACT COMPLETE failures=",failures)
    scene.queue_free()
