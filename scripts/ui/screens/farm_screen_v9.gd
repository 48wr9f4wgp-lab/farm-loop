class_name FarmScreenV9
extends "res://scripts/ui/screens/farm_screen_v8.gd"

const FarmDioramaV9Class = preload("res://scripts/ui/farm_diorama_v9.gd")

func _build_map(host) -> void:
    var farm_map = FarmDioramaV9Class.new()
    var viewport_h: float = host.get_viewport_rect().size.y
    # Slightly shorter hero improves portrait aspect and gives the 3D camera
    # more horizontal breathing room without making the diorama feel small.
