class_name FarmScreenV13
extends "res://scripts/ui/screens/farm_screen_v12.gd"

const FarmDioramaV13Class = preload("res://scripts/ui/farm_diorama_v13.gd")

# Portrait polish pass: early FTUE stays visually single-purpose.
# The restore card is introduced only when restoration becomes the active beat,
# and the guided hero is slightly shorter so it never fights the fixed nav.

func _build_map(host) -> void:
    var farm_map = FarmDioramaV13Class.new()
    ...
    if guided:
        farm_map.custom_minimum_size = Vector2(0,clampf(viewport_h * 0.47,400.0,420.0))
    else:
        farm_map.custom_minimum_size = Vector2(0,clampf(viewport_h * 0.50,440.0,500.0))
...
func _build_restore_status(host) -> void:
    var guided: bool = host.ftue_service != null and host.ftue_service.active(host.state)
    if guided:
        var guided_step: int = host.ftue_service.step(host.state)
        # Before the player returns compost to the soil, the recovery card is
        # premature information and caused an ugly sliver behind the bottom nav.
        if guided_step < 4:
            return
    super._build_restore_status(host)
